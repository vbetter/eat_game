        ...
        window.plugin = new window.Vue({
            ...
            methods: {
                    ...
                    for (let i = 3; i < excelData.length; i++) {
                        ...
                        if (lineData.length === 0) {
                            ...
                            if (lineData.length < title.length) {
                                ...
                                //this._addLog("[Error] 发现第" + i + "行缺少字段,跳过该行数据配置 "+ itemSheet.name + "*" + itemSheet.sheet );
                                //this._addLog( "--->lineData.length:"+lineData.length + " ,title.length:"+title.length)

                                let zlength = title.length- lineData.length;
                                for (var z =0 ;z<zlength;z++)
                                {
                                    lineData.push("");
                                }
                                //this._addLog(title)
                                //this._addLog(lineData)
                                //continue;
                            } else if (lineData.length > title.length) {
                                this._addLog("[Error] 发现第" + i + "行多余字段,跳过该行数据配置."+ itemSheet.name + "*" + itemSheet.sheet);
                                continue;
                            }
                        }
                        let saveLineData = {};
                        ...
                        for (let j = 1; j < title.length; j++) {
                            ...
                            if (canExport) {
                                ...
                                if (value === undefined) {
                                    value = "";
                                    //this._addLog("[Warn] 发现空单元格:" + itemSheet.name + "*" + itemSheet.sheet + " => (" + key + "," + (i + 1) + ")");
                                }
                ...
                onBtnClickGen() {
                    ...
                    let jsSavePath3 = path.join(Editor.projectInfo.path, "assets/Configs");
                    fsExtra.emptyDirSync(jsSavePath1);
                    fsExtra.emptyDirSync(jsSavePath2);
                    fsExtra.emptyDirSync(jsSavePath3);
                                        ...
                                        let writeFileJs = function (savePath, isClient) {
                                            ...
                                            if (Object.keys(sheetJsData).length > 0) {
                                                if (this.isMergeJavaScript) {
                                                    ...
                                                    //在复制一份到前端工程目录
                                                    let savePath_client = path.join(Editor.projectInfo.path, "assets/Configs");
                                                    this._addLog("-------savePath_client:"+savePath_client)
                                                    let fileNameFullPath_client = path.join(savePath_client, itemSheet.sheet + ".js");
                                                    this._onSaveJavaScriptCfgFile(fileNameFullPath_client, sheetJsData);
                                                }
                                            }
                                        }.bind(this);
